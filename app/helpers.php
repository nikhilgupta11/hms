...
{
     //Prescriptions module for user
    if ($menu == User::PRESCRIPTIONS_USER) {
    ...
    if ($menu == User::MAIN_BED_MGT) {
     ...
     //document module For Receptionist
    if ($menu == User::DOCUMENT_FOR_RECEPTIONIST) {
        $defaultRoute = route('documents.index');
        $subMenus = ['Documents', 'Document Types'];
    }
    //billing module For Receptionist
    if ($menu == User::INVOICE_FOR_RECEPTIONIST) {
        $defaultRoute = route('invoices.index');
        $subMenus = ['Invoices', 'Bills'];
    }
    //billing module
