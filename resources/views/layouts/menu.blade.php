...
@if(Auth::user()->email_verified_at != null)
    ...
    {{-- Invoice  --}}
    <?php
        $billingAccountMGT = getMenuLinks(\App\Models\User::INVOICE_FOR_RECEPTIONIST);
    ?>
    @if ($billingAccountMGT)
        <li class="nav-item {{ Request::is('invoices*','bills*') ? 'active' : '' }}">
    ...
    <?php
        $document_res = getMenuLinks(\App\Models\User::DOCUMENT_FOR_RECEPTIONIST);
    ?>
    @if ($document_res)
      <li class="nav-item {{ Request::is('documents*','document-types*') ? 'active' : '' }}">
        <a class="nav-link  d-flex align-items-center py-3" href="{{ $document_res }}">
            <span class="aside-menu-icon pe-3"><i class="fas fa-file"></i></span>
            <span class="aside-menu-title">{{ __('messages.documents') }}</span>
            <span class="d-none">{{__('messages.document_types')}}</span>
        </a>
...
<?php
$prescriptions = getMenuLinks(\App\Models\User::PRESCRIPTIONS_USER)
?>
@if ($prescriptions)
    <li class="nav-item  {{ Request::is('prescriptions*') ? 'active' : '' }}">
        <a class="nav-link  d-flex align-items-center py-3" href="{{ $prescriptions }}"> <span class="aside-menu-icon pe-3"><i class="fas fa-prescription"></i></span>
            <span class="aside-menu-title">{{__('messages.prescriptions')}}</span>
        </a>
    </li>
@endif-->
